...
import { collection, query, where, onSnapshot, addDoc, deleteDoc, updateDoc, doc, serverTimestamp, writeBatch, getDocs, setDoc, arrayUnion, arrayRemove } from 'firebase/firestore';
...
const HomePage = ({ db, storage, user, userProfile, onSelectProject, onSelectUser, isAdmin, requestStatus }) => { 
  ...
  // --- VECCHIE AZIONI VIAGGIO ---

  const handleSubscribe = async (trip) => { 
    setSubscribing(true); 
    try { 
        const projectRef = await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'projects'), { 
            name: trip.name, description: trip.description, createdBy: user.uid, createdAt: serverTimestamp(), memberIds: [user.uid], basedOn: trip.id, location: trip.location || '', dateStart: trip.dateStart || '', dateEnd: trip.dateEnd || '', price: trip.price || 0, maxPeople: trip.maxPeople || 0, imageUrl: trip.imageUrl || '', status: 'private', creatorName: trip.creatorName || trip.organizers?.[0] || 'Sconosciuto', organizers: trip.organizers || [], joinType: 'open', joinDeadline: trip.joinDeadline || '', ratingTotal: trip.ratingTotal || 0, ratingCount: trip.ratingCount || 0, permissions: trip.permissions || { details: { visible: true, interact: true }, todo: { visible: true, interact: true }, memories: { visible: true, interact: true } } 
        }); 
        await setDoc(doc(collection(projectRef, 'participants'), user.uid), { name: userProfile.name }); 
        const exSnapshot = await getDocs(collection(db, 'artifacts', appId, 'public', 'data', 'published_trips', trip.id, 'excursions')); 
        const batch = writeBatch(db); 
        exSnapshot.forEach(docSnap => { batch.set(doc(collection(projectRef, 'excursions')), docSnap.data()); }); 
        await batch.commit(); 
        setView('mine'); 
    } catch (err) { console.error(err); alert(err.message); } finally { setSubscribing(false); } 
  };
